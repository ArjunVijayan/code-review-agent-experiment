...
The `pr-code-review` skill conducts the complete evidence-based code review. It first refreshes incremental historical intelligence, then collects Git-only context for either `{ "base": "main", "source": "dev/bug_fix" }` or a GitHub PR/MR URL. GitHub URLs resolve their refs automatically; other providers can supply normalized `base_ref` and `source_ref` metadata. It creates a structured `CodeReviewContext`, renders evidence packages, packages them into a semantic-analysis input, evaluates five review gates, and produces the auditable `review/review-result.json` plus `review/review-report.html` used to make approval decisions.
