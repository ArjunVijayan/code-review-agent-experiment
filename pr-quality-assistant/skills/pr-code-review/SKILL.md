...
4. Pass normalized provider JSON to `scripts/discover-historical-prs.py` with the state file and `--limit 100`. By default, the script resolves `.code-review/insights-state.json` and `review/change-requests.json` from the Git repository root, even when invoked from the scripts directory. It returns only newly discovered, merged requests for the selected base. On a changed base ref, start a fresh analyzed set. If provider input is absent, it returns no requests with `provider_data_available: false` and does not fail bootstrap.
