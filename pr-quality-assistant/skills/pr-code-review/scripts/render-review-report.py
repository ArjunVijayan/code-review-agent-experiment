...
def render(result: dict) -> str:
    ...
    decision = html.escape(result.get("decision", "NOT_READY_TO_MERGE"))
    ...
    def issue_row(item: dict) -> str:
        ...
        return (
            ...
            f"<td>{text(item.get('category'))}</td>"
            f"<td>{text(item.get('gate', gates_text))}</td>"
            f"<td>{text(item.get('status', 'FAIL'))} / {'Blocking' if item.get('blocking') else 'Non-blocking'}</td>"
            f"<td>{text(item.get('issue'))}</td>"
            f"<td>{text(item.get('impact'))}<br><strong>Expected:</strong> {text(item.get('expected'))}<br><strong>Actual:</strong> {text(item.get('actual'))}<br><strong>Evidence:</strong> {text(item.get('evidence'))}</td>"
            f"<td>{text(item.get('recommendation'))}</td>"
            f"<td>{text(item.get('confidence'))}</td>"
            f"<td>{reference_cell(item)}</td></tr>"
        )

    issues = "".join(
        issue_row(item) for item in result.get("findings", [])
    ) or '<tr><td colspan="9">No issues reported.</td></tr>'
...
<body><h1>PR Merge Readiness</h1><div class="meta"><p>PR/MR: {request_link}</p><p>Base: {text(pull_request.get('base'))} | Source: {text(pull_request.get('source'))}</p><p class="status {status}">FINAL DECISION: {decision}</p><p class="summary"><strong>What this means:</strong> {html.escape(primary_reason)}<br><strong>Issues:</strong> {len(findings)} total, {len(blocking_findings)} blocking, {len(findings) - len(blocking_findings)} non-blocking.</p></div>
...
<h2 id="issues">Issues and Evidence</h2><p class="muted">Each issue connects its gate to impact, expected versus actual behavior, evidence, remediation, confidence, and a source reference.</p><table><thead><tr><th>ID</th><th>Severity</th><th>Category</th><th>Gate(s)</th><th>Status / Blocking</th><th>What is wrong</th><th>Impact / expected / actual / evidence</th><th>Recommended change</th><th>Confidence</th><th>Reference</th></tr></thead><tbody>{issues}</tbody></table>
